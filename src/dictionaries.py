...
class IndexDictionary:
    ...
    # usage: index the sentence
    def index_sentence(self, sentence):
        return [self.token_to_index(token) for token in sentence]

    # usage: transfer indexes to original token
    def tokenize_indexes(self, token_indexes):
    ...
    def _build_vocabulary(self, iterable, vocabulary_size):
        ...
        if vocabulary_size is not None:
            # list {vocabulary_size - len(self.special_tokens)} most common words for use
    ...
    def load(cls, data_dir, mode='shared', vocabulary_size=None):
        ...
        vocab_tokens = {}
        token_counts = []
        with open(vocabulary_filepath, encoding='utf8') as file:
            for line in file:
